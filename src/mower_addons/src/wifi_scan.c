...
{   
	...
	if(channel->buf == NULL) die("Insufficent memory for netlink socket buffer");
		

	channel->ifindex=if_nametoindex(interface);

	if(channel->ifindex==0) die_errno("Incorrect network interface");
...
static void init_netlink_socket(struct netlink_channel *channel)
{
	...
	if (channel->nl == NULL) die_errno("mnl_socket_open");
		

	if (mnl_socket_bind(channel->nl, 0, MNL_SOCKET_AUTOPID) < 0) die_errno("mnl_socket_bind");
